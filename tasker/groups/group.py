...
from tasker.common.exceptions import StopTaskGroupException
...
class Coroutine(Greenlet):
    ...
    def init_run(self, context: CoroutineContext) -> None:
        ...
        context.coroutine_group = self.task_group
    ...
    def _run(self):
        ...
        try:
            ...
            while self.running:
                sampler = self.task_group_main_controller.next()

                while self.running and sampler is not None:
                    log.debug(
                        f'coroutine:[ {self.coroutine_name} ] '
                        f'next sampler:[ {sampler.name if isinstance(sampler, TaskElement) else None} ]'
                    )
                    # 处理Sampler
                ...
                if self.task_group_main_controller.done:
                    self.running = False
                    log.info(f'协程:[ {self.coroutine_name} ] 循环迭代已结束')

        except StopTaskGroupException:
            log.debug(f'coroutine:[ {self.coroutine_name} ] except StopTaskGroupException Exception, stoping coroutine')
    ...
    def __continue_on_coroutine_loop(self) -> None:
        ...
        self.task_group_main_controller.start_next_loop()
    ...
    def __get_parent_controllers(self, sampler: Sampler) -> list:
        """获取Sampler的父控制器节点"""
        find_test_elements_up_to_root = FindTaskElementsUpToRoot(sampler)
    ...
    def stop_coroutine_group(self) -> None:
        ...
        self.task_group.stop_coroutines()

    def stop_test(self) -> None:
        log.info(f'协程发起 Stop Task Collection请求，协程名称:[ {self.coroutine_name} ] ')
        self.running = False
        if self.engine:
            self.engine.stop_test()

    def stop_test_now(self) -> None:
        log.info(f'协程发起 Stop Task Collection Now 请求，协程名称:[ {self.coroutine_name} ] ')
        self.running = False
        if self.engine:
    ...
    def __remove_samplers_and_controllers(elements: list) -> None:
        ...
        for element in elements[:]:
            ...
            if isinstance(element, Controller):
                elements.remove(element)
            if not isinstance(element, TaskElement):
                elements.remove(element)
