...
from tasker.groups.package import SamplePackage
...
class TaskCompiler(HashTreeTraverser):
    ...
    def configure_sampler(self, sampler) -> SamplePackage:
        ...
        sampler.clear_task_element_children()
        for config in package.configs:
            from tasker.configs.config import NoConfigMerge
            if not isinstance(config, NoConfigMerge):
                sampler.add_task_element(config)
        return package
    ...
    def __compile_controller(self, node, subtree):
        if node not in self.compiled_node:
            ...
            # 递归编译子代节点
            compiler = TaskCompiler(parent_level_elements)
...
class FindTaskElementsUpToRoot(HashTreeTraverser):
