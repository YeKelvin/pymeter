...
from taskmeter.common.exceptions import EngineException
...
from taskmeter.engine.interface import TestStateListener
from taskmeter.groups.context import ContextService
from taskmeter.groups.group import TaskGroup
from taskmeter.listeners.result_collector import ResultCollector
...
class StandardEngine(Greenlet):
    ...
    def _run(self) -> None:
        ...
        # 存储 collection层一级子代的节点(非TaskGroup节点)
        test_level_elements = self.tree.index(0).list()
        self.__remove_coroutine_groups(test_level_elements)  # 删除 TaskGroup节点
    ...
    def __start_coroutine_group(self,
        ...
        Args:
            ...
            test_level_elements:    collection层一级子代的节点（非TaskGroup节点）
        ...
        try:
            ...
            # 把 collection层一级子代的节点（非TaskGroup节点）添加至 group层
    ...
    def __remove_coroutine_groups(elements: list) -> None:
        ...
        for node in elements[:]:
            if isinstance(node, TaskGroup) or not isinstance(node, TestElement):
