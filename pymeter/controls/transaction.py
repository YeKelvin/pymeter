...
class TransactionController(GenericController):
    ...
    def next_with_transaction_sampler(self):
        ...
        if self.transaction_sampler and self.transaction_sampler.done:
            ...
            logger.debug(f'线程:[ {self.ctx.coroutine_name} ] transaction:[ {self.name} ] end of transaction')
            logger.debug(f'线程:[ {self.ctx.coroutine_name} ] 控制器:[ {self.name} ] next:[ None ]')
            return None

        # Check if it is the start of a new transaction
        if self.first:  # must be the start of the subtree
            logger.debug(f'线程:[ {self.ctx.coroutine_name} ] transaction:[ {self.name} ] start of transaction')
...
class TransactionSampler(Sampler):
    ...
    def set_done(self):
        logger.debug(f'线程:[ {self.controller.ctx.coroutine_name} ] transaction:[ {self.controller.name} ] done')
        self.done = True
